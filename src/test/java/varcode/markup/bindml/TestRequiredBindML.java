...
{
    /**
     * Verify that an attempt to tailor the toString will fail with a precise
 RequiredButNull Exception
     *
    ...
    private static void verifyThrowsVarRequired( String mark )
    {
        try
        {
            Author.toString( BindML.compile( mark ), VarContext.of() );
    ...
    public void testTags()
    {
        ...
        assertEquals( "", Author.toString( BindML.compile( "{_+:{+fieldType+} {+fieldName+}*+_}" ), VarContext.of() ) );
