 ...
 * and import the classes appropirately)
 * 
	...
	implements SelfAuthored
{
	...
	/** 
	 * A List of "generic" objects that are convertable to a sequence
	 * of code.  contains:
	 * <UL>
	 *   <LI>Strings
	 *   <LI>_code
	 ...
	 */
	private List<_class.simpleName> catchException = 
		new ArrayList<_class.simpleName>();
	...
	{
		if( codeComponents == null || codeComponents.size() == 0 )
		{
			return null;
		}
	...
	public _code replace( String target, String replacement )
	{
		...
		for( int i = 0; i < this.catchException.size(); i++ )
		{
			this.catchException.set( i, 
				(_class.simpleName)_class.simpleName.of( 
					this.catchException.get( i ).toString().replace( target, replacement ) ) );
		}
		return this;
	}
	...
	{
		this.catchException.add( new _class.simpleName( catchException ) );
	...
	{
		this.catchException.add( new _class.simpleName( simpleExceptionName ) );
		for( int i = 0; i < handleCode.length; i++ )
		{
			this.handleException.add( _code.of( handleCode[ i ] ) );
		}
